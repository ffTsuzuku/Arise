...
- **Install AI-agent README** in the main menu calls the existing `lib/skill.js` installer after choosing global or current-directory scope. It installs the Arise `SKILL.md` guide, returns to the menu after installation, and performs no installation when cancelled.
- Plugin menu actions can supply `label`, `description`/`hint`, `group`, `shortcut`, `danger`, and `subtle` presentation metadata. Ungrouped plugins appear under **Extensions**. Reserve `l`, `s`, `c`, `a`, and `q` for built-in actions and j/k for navigation; the worktree plugin uses `n`, `o`, `w`, and `x`. A create/switch/open handler returns `true` when it has opened a session and the menu should finish; cancelling keeps the menu open.
