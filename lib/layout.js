...
/**
 * Render a declarative layout onto a terminal multiplexer (tmux or Herdr)
 * 
 * @param {Object} options
 * @param {Array} options.layout Array of pane definitions
 * @param {string} options.rootPaneId ID of the root pane created with workspace/session
 ...
 * @param {Object} [options.driver] Active terminal multiplexer driver (tmux or herdr)
 */
function renderLayout({ layout = DEFAULT_QUADRANT_LAYOUT, rootPaneId, cwd, focusTarget = 'agy', driver = null }) {
  const activeDriver = driver || resolveDriver();
  const paneMap = new Map(); // id -> paneId
  let targetFocusPaneId = null;

  logger.debug(`Rendering layout (${layout.length} panes) with driver="${activeDriver.name}" rootPaneId="${rootPaneId}":`, {
    cwd,
    focusTarget,
  ...
  for (const paneDef of layout) {
    ...
    if (paneDef.position === 'root' || !paneDef.from) {
      ...
      try {
        currentPaneId = activeDriver.splitPane({
          paneId: parentPaneId,
    ...
    if (currentPaneId) {
      ...
      if (paneDef.title) {
        activeDriver.renamePane(currentPaneId, paneDef.title);
      }

      if (paneDef.cmd) {
        activeDriver.runInPane(currentPaneId, paneDef.cmd);
      }

      if (isFocusMatch(paneDef)) {
        targetFocusPaneId = currentPaneId;
        if (typeof activeDriver.focusPane === 'function') {
          activeDriver.focusPane(currentPaneId);
        }
      }
    }
  }
