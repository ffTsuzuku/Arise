...
async function executeNuke(flags, config, cwd = process.cwd()) {
  ...
  if (!target) {
    ...
    if (matchingWt) {
      ...
      config.repo.worktreesBase &&
      currentCwd !== path.resolve(repoRoot) &&
      (currentCwd === path.resolve(config.repo.worktreesBase) || currentCwd.startsWith(path.resolve(config.repo.worktreesBase) + path.sep))
    ) {
      const rel = path.relative(config.repo.worktreesBase, currentCwd);
      const topDir = rel.split(path.sep)[0];
      if (topDir && topDir !== '.' && topDir !== '..') {
        target = path.join(config.repo.worktreesBase, topDir);
        console.log(`Auto-detected leftover worktree directory: "${target}"`);
      } else {
  ...
  if (fs.existsSync(targetWorktreePath)) {
    ...
    try {
      ...
      deleted = !fs.existsSync(targetWorktreePath);
      if (deleted) {
        console.log(`Directory deleted successfully.`);
      }
    } catch (err) {
      if (process.platform !== 'win32') {
        console.warn(`Standard directory removal failed (${err.message}). Attempting elevated removal with sudo...`);
        try {
          execSync(`sudo rm -rf "${targetWorktreePath}"`, { stdio: 'inherit' });
          deleted = !fs.existsSync(targetWorktreePath);
          if (deleted) {
            console.log(`Directory deleted successfully with sudo.`);
          } else {
            console.error(`Failed to delete directory "${targetWorktreePath}" with sudo.`);
          }
        } catch (sudoErr) {
          console.error(`Failed to delete directory "${targetWorktreePath}" with sudo: ${sudoErr.message}`);
        }
      } else {
    ...
    if (!deleted && fs.existsSync(targetWorktreePath) && process.platform !== 'win32') {
      try {
        console.warn(`Worktree directory still exists. Attempting elevated removal with sudo...`);
        execSync(`sudo rm -rf "${targetWorktreePath}"`, { stdio: 'inherit' });
        if (!fs.existsSync(targetWorktreePath)) {
          console.log(`Directory deleted successfully with sudo.`);
        }
