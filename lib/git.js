...
function getRepoRootDir(cwd = process.cwd(), bareRepo = null) {
  ...
  try {
    ...
    try {
      ...
      const parentDir = path.dirname(resolvedCwd);
      const currentDirName = path.basename(resolvedCwd);
      const possibleSibling = currentDirName.replace(/-worktrees$/, '.git');
      if (possibleSibling !== currentDirName && fs.existsSync(path.join(parentDir, possibleSibling)) && isBareRepo(path.join(parentDir, possibleSibling))) {
        return path.join(parentDir, possibleSibling);
      }
      const grandParentDir = path.dirname(parentDir);
      const parentDirName = path.basename(parentDir);
      const possibleSiblingFromGrandParent = parentDirName.replace(/-worktrees$/, '.git');
      if (possibleSiblingFromGrandParent !== parentDirName && fs.existsSync(path.join(grandParentDir, possibleSiblingFromGrandParent)) && isBareRepo(path.join(grandParentDir, possibleSiblingFromGrandParent))) {
        return path.join(grandParentDir, possibleSiblingFromGrandParent);
      }
    } catch (e) {}
...
function removeWorktree(worktreePath, options = {}) {
  ...
  try {
    ...
    const worktrees = getWorktrees(options);
    const stillRegistered = worktrees.some(wt => path.resolve(wt.path) === path.resolve(worktreePath));
    if (stillRegistered) {
