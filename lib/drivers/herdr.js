...
const os = require('os');
const { promptConfirm, isInteractive, p } = require('../tui/prompt');
...
function getRecommendedInstallCommand() {
  ...
  if (process.platform === 'win32') {
    return {
      ...
      docsUrl: 'https://herdr.dev/install/',
    };
  }
  return {
...
async function ensureHerdrInstalled(options = {}) {
  ...
  if (!autoConfirm && (!isInteractive() || process.env.CI)) {
    p.log.error(`\n[arise] Error: "herdr" is required to manage terminal workspaces but was not found in PATH.`);
    p.log.error(`Please install Herdr before running arise:\n  ${cmd}\n`);
    p.log.error(`Docs: ${docsUrl || 'https://herdr.dev/docs/install/'}`);
    process.exit(1);
  }

  if (!autoConfirm) {
    p.log.info(`\n[arise] "herdr" is required to manage terminal workspaces but was not found in PATH.`);
    p.intro('Install Herdr');
    const answer = await promptConfirm({
      message: `Install Herdr via ${name}?`,
      hint: cmd,
      defaultYes: true,
    });
    if (!answer) {
  ...
  if (result.status !== 0 || !isHerdrInstalled()) {
    ...
    p.log.error(`Please visit https://herdr.dev/docs/install/ to install manually.`);
    process.exit(1);
  }

  p.log.info(`[arise] Herdr installed successfully!\n`);
  return true;
}
...
function runInPane(paneId, command) {
  ...
  try {
    ...
    p.log.error(`Failed to execute command "${command}" in pane ${paneId}: ${e.message}`);
  }
}
...
function closeWorkspacesMatching(matchTargets = []) {
  ...
  for (const ws of matchingWorkspaces) {
    if (ws.workspace_id) {
      p.log.info(`Closing Herdr workspace "${ws.label}" (ID: ${ws.workspace_id}) and terminating panes...`);
      closeWorkspace(ws.workspace_id);
      p.log.info(`Herdr workspace "${ws.label}" closed.`);
    }
  }
}
...
function attachOrSwitchSession(sessionName) {
  if (process.env.HERDR_ENV === '1') {
    p.log.info(`Already inside herdr. Workspace "${sessionName}" created and focused!`);
  } else {
    p.log.info(`Starting herdr session...`);
...
const herdrDriver = {
  name: 'herdr',
  isAvailable: isHerdrInstalled,
  ensureInstalled: ensureHerdrInstalled,
  listSessions() {
    const raw = listWorkspaces();
    return raw.map((w) => ({
      id: w.workspace_id,
      name: w.label,
      label: w.label,
  ...
  createSession({ name, cwd }) {
    const res = createWorkspace({ label: name, cwd });
    return {
      sessionId: res.workspaceId,
      rootPaneId: res.rootPaneId,
      name,
    };
  },
  closeSession(nameOrId) {
    const workspaces = listWorkspaces();
    const match = workspaces.find((w) => w.workspace_id === nameOrId || w.label === nameOrId);
    const targetId = match ? match.workspace_id : nameOrId;
    return closeWorkspace(targetId);
  },
  closeSessionsMatching(targets) {
    return closeWorkspacesMatching(targets);
  },
  focusSession(sessionId) {
    return focusWorkspace(sessionId);
  },
  splitPane({ paneId, direction = 'right', cwd, focus = false }) {
    return splitPane({ paneId, direction, cwd, focus });
  },
  renamePane(paneId, name) {
    return renamePane(paneId, name);
  },
  runInPane(paneId, command) {
    return runInPane(paneId, command);
  },
  focusPane(paneId) {
    // Herdr does not have an explicit pane focus command other than split focus
  },
  attachOrSwitchSession(sessionName) {
    return attachOrSwitchSession(sessionName);
  },
