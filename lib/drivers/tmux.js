...
function closeSessionsMatching(matchTargets = []) {
  ...
  for (const s of matching) {
    console.log(`Closing tmux session "${s.name}" and terminating panes...`);
    closeSession(s.name);
    console.log(`tmux session "${s.name}" closed.`);
  }
}
...
function runInPane(paneId, command) {
  ...
  try {
    ...
    console.error(`Failed to send command to tmux pane ${paneId}: ${e.message}`);
  }
}
...
function attachOrSwitchSession(sessionName) {
  if (process.env.TMUX) {
    console.log(`Already inside tmux. Switching to session "${sessionName}"...`);
