...
`;
  ...
  static async runCreatePresetWizard(cwd, options = {}) {
    ...
    // 4. Auto-Detection File Marker (Optional)
    let detectMarker = options.detectMarker;
    if (detectMarker === undefined && process.stdin.isTTY) {
      detectMarker = await promptText({
        message: 'Optional project file marker for auto-detection (e.g. "Cargo.toml", "manage.py", "go.mod", "package.json" or Enter for none):',
        defaultValue: '',
      });
      detectMarker = (detectMarker || '').trim();
    }

    // 5. Setup Shell Commands
    ...
    // 7. Layout Template & Customization
    ...
    const detectFnCode = detectMarker
      ? `detect(cwd) {\n    return fs.existsSync(path.join(cwd, '${detectMarker}'));\n  },`
      : `detect(cwd) {\n    return false;\n  },`;

    const presetCode = `const fs = require('fs');
const path = require('path');
