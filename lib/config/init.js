...
class ConfigInitWizard {
  static async run(options = {}) {
    ...
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Configuration Setup Wizard'))}`);
    ...
    if (!isQuick && process.stdin.isTTY && !options.local && !options.global) {
        ...
        choices: [
          {
            ...
            value: 'preset',
            hint: 'Define stack layout, setup commands, and save globally or locally',
          },
          {
      ...
      const modeChoice = await promptSelect({
        ...
        choices: [
          ...
          {
            label: 'Guided Step-by-Step Setup',
            value: 'guided',
    ...
    if (!options.local && !options.global) {
      if (isInsideGit) {
        if (isQuick) {
          ...
          clearScreen();
          p.intro(`${pc.bold(pc.cyan('Arise Configuration Setup Wizard'))}`);
          p.log.step(pc.bold(pc.cyan('Configuration Scope')));
...
// • setup: Shell commands run upon workspace creation
// • cleanup: Shell commands run before workspace deletion
...
`;
  ...
  static async runGuidedInit(cwd, isLocal, isInsideGit, options = {}) {
    ...
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 1 of 6: Project Preset')}`);
    ...
    choices.push({
      label: 'Custom Preset Name or File Path...',
      value: '__custom__',
    ...
    // Detected git topology defaults (resolved silently without interrupting user)
    const defaultCandidate = localBranches.includes('develop')
      ? 'develop'
      : (localBranches.includes('main') ? 'main' : (localBranches[0] || 'main'));

    // Step 2: AI CLI Agent & Workspace Focus
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 2 of 6: AI Agent Integration')}`);
    ...
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 2 of 6: Workspace Settings')}`);
    ...
    // Step 3: Terminal Layout & Workspace Panes (layout)
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 3 of 6: Terminal Layout')}`);
    ...
    // Step 4: Workspace Lifecycle Commands (setup & cleanup)
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 4 of 6: Lifecycle Commands')}`);
    ...
    // Step 5: Agent Skill Installation
    let skillInstalled = false;
    if (process.stdin.isTTY) {
      clearScreen();
      p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 5 of 6: AI Agent Skill')}`);
    ...
    clearScreen();
    p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Step 6 of 6: Save Configuration')}`);
    ...
    if (isInsideGit) {
      configObj.repo = {
        defaultBaseBranch: defaultCandidate,
        protectedBranches: ['main', 'master', 'develop', 'prod', 'staging'],
      };
      if (isBare) {
        configObj.repo.bareRepo = repoRoot;
...
`;
  ...
  static async promptLayoutAgnostic({
    ...
    if (!layoutChoice && process.stdin.isTTY) {
      clearScreen();
      p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Terminal Layout Architecture')}`);
      console.log(`  ${ANSI.bold}Available Layout Architectures:${ANSI.reset}`);
      console.log(`  ${ANSI.cyan}• Custom Pane-by-Pane Builder:${ANSI.reset}  Interactively define every pane, startup command, parent, and split direction.`);
      ...
      layoutChoice = await promptSelect({
        ...
        choices: [
          {
            label: 'Custom Pane-by-Pane Builder',
            value: 'custom',
    ...
    if (layoutChoice === 'custom') {
      ...
      if (process.stdin.isTTY) {
        clearScreen();
        p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Custom Pane Layout Builder')}`);
        ...
        const paneCountInput = await promptText({
          ...
          defaultValue: '4',
          validate: (val) => {
            const raw = typeof val === 'string' ? val : (val ? String(val) : '4');
      ...
      for (let i = 1; i <= paneCount; i++) {
        ...
        if (process.stdin.isTTY) {
          clearScreen();
          p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan(`Custom Layout — Configure Pane ${i} of ${paneCount}`)}`);
          ...
          const paneIdInput = await promptText({
            ...
            validate: (val) => {
              ...
              return Boolean(str) || 'Pane ID cannot be empty';
            },
          });
        ...
        if (i === 1) {
          ...
          if (process.stdin.isTTY) {
            ...
            const chosenSplitDir = await promptSelect({
              ...
              choices: [
                { label: 'Right (Vertical split - side-by-side)', value: 'right' },
                { label: 'Down (Horizontal split - stacked)', value: 'down' },
              ],
      ...
      for (let i = 0; i < templatePanes.length; i++) {
        ...
        if (process.stdin.isTTY) {
          clearScreen();
          p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan(`Configure Pane Commands (${i + 1} of ${templatePanes.length})`)}`);
    ...
    if (configuredLayout.length > 0) {
      ...
      if (process.stdin.isTTY) {
        clearScreen();
        p.intro(`${pc.bold(pc.cyan('Arise Setup'))} ${pc.dim('•')} ${pc.cyan('Default Workspace Focus')}`);

        const focusChoices = configuredLayout.map((p) => ({
          label: `${p.title} (${p.id})`,
          hint: p.isAgent ? 'AI agent' : (p.cmd || 'shell'),
          value: p.id,
        }));
  ...
  static async runCreatePresetWizard(cwd, options = {}) {
    clearScreen();
    p.intro(pc.bold(pc.cyan('Create Reusable Preset')));
    ...
    // 2. Icon / Identifier
    const icon = options.icon || options.emoji || 'terminal';

    // 3. Storage Scope (Global vs Local)
    let exportScope = options.exportScope;
    if (!exportScope && process.stdin.isTTY) {
      clearScreen();
      p.intro(`${pc.bold(pc.cyan('Arise Preset Wizard'))} ${pc.dim('•')} ${pc.cyan('Storage Scope')}`);
...
`;
    ...
    clearScreen();
    const iconTag = icon && icon !== 'terminal' ? ` (${icon})` : '';
    p.log.success(`Preset "${presetName}"${iconTag} saved successfully!`);
  ...
  static async handleExportPreset({
    ...
    if (typeof wantExport !== 'boolean' && process.stdin.isTTY) {
      clearScreen();
      p.intro(pc.bold(pc.cyan('Export Reusable Custom Preset')));
    ...
    const exportIcon = options.icon || options.emoji || 'terminal';
...
`;
    ...
    const exportIconTag = exportIcon && exportIcon !== 'terminal' ? ` (${exportIcon})` : '';
    p.log.success(`Standalone custom preset "${exportName}"${exportIconTag} saved successfully!`);
    console.log(`  ${ANSI.dim}Preset location: ${exportFilePath}${ANSI.reset}\n`);
    return exportFilePath;
  }
  ...
  static printSuccessCard(filePath, message) {
    ...
    console.log(`${pc.gray('│')}  ${pc.dim('Config path:')} ${pc.cyan(filePath)}`);
    console.log(`${pc.gray('│')}`);
    console.log(`${pc.gray('│')}  ${pc.bold('Next Steps:')}`);
    console.log(`${pc.gray('│')}    ${pc.cyan('arise --branch <feature>')}  ${pc.dim('Create git worktree & workspace')}`);
    console.log(`${pc.gray('│')}    ${pc.cyan('arise')}                   ${pc.dim('Launch interactive menu')}`);
    console.log(`${pc.gray('│')}    ${pc.cyan('arise --nuke')}            ${pc.dim('Tear down worktrees safely')}`);
    console.log(`${pc.gray('│')}    ${pc.cyan('arise --help')}            ${pc.dim('View all CLI options')}`);
