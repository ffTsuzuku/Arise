const fs = require('fs');
const { paint, printSummary } = require('./tui/theme');
...
Usage:
  ...
  arise init [preset] [options]         Review repository settings / Create a reusable preset
...
Interactive & Wizard Modes:
  ...
  arise init, --init                   Choose a preset, review settings, and save repository configuration.
  arise init preset, arise preset new  Open the separate reusable preset builder.
  --quick, -q                          (Wizard option) Save selected/detected defaults without review.
  --global                             (Wizard option) Configure user defaults or store a preset globally.
  --local                              (Wizard option, default) Configure this repository/worktree.
  --target, --out <path>               (Wizard option) Write configuration to a specific JSON file.
...
`;
  printSummary('Command reference');
  let section = 0;
  console.log(usage.split('\n').map((line) => {
    if (/^[A-Z].*:$/.test(line)) {
      return paint(`${String(++section).padStart(2, '0')} / ${line.slice(0, -1).toUpperCase()}`, 'muted');
    }
    return line;
  }).join('\n'));
}
