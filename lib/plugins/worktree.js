...
const { loadConfigFile, resolveConfiguration } = require('../config');
const {
  clearScreen,
  promptSelect,
  promptMultiSelect,
    ...
    async resolveTarget(context) {
      ...
      if (!worktreeExists && config.configFile && fs.existsSync(config.configFile)) {
        try {
          ...
          const isJsonConfig = config.configFile.endsWith('.json') || config.configFile.endsWith('.ariserc');
          const fileConfig = isJsonConfig ? loadConfigFile(config.configFile, { strict: true }) : null;
          const preset = fileConfig?.preset;
          const isRelativePreset = typeof preset === 'string' && !path.isAbsolute(preset) &&
            !preset.startsWith('~') && (preset.startsWith('./') || preset.startsWith('../') || /\.(c?js)$/.test(preset));
          if (isRelativePreset) {
            // The preset may be ignored by Git, so keep referencing its source location.
            fileConfig.preset = path.resolve(path.dirname(config.configFile), preset);
            fs.writeFileSync(targetConfigPath, `${JSON.stringify(fileConfig, null, 2)}\n`, 'utf8');
          } else {
